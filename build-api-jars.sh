...
echo "Make build"
bazel build -c opt --config=darwin scripts/packages:tarpkgs
bazel build -c opt --config=darwin scripts/packages:binpkgs

echo "Make jar components"
bazel build --config=darwin heron/api/src/java:all
bazel build --config=darwin heron/spi/src/java:all
bazel build --config=darwin storm-compatibility/src/java:all
