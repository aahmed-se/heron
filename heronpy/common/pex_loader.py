...
def resolve_heron_suffix_issue(abs_pex_path, class_path):
  ...
  ``heronpy.common.dep_module.DepClass`` for example, pex_loader does not guarantee that
  ``DepClass` is imported correctly. This is because ``heronpy.common.dep_module`` is not
  explicitly added to sys.path, while ``heronpy.common`` module exists as the native heron
